...
# 编码防错清单（常见致命错误，每次编码前必查！）

## scipy 陷阱
- `scipy.optimize.minimize` 只做最小化；最大化利润/覆盖率/得分时**必须对目标函数取负**，并在结果记录中还原真实目标值。
- `scipy` 不等式约束方向是 `fun(x) >= 0`；容量约束、下界约束、预算约束最容易写反，写完要代入几个边界点检查符号。
- **不要只相信求解器的 `success`**。最优解必须重新代入所有约束函数，输出每条约束的值、边界、松弛量和是否活跃。

## 整数变量
- 整数变量、0-1 变量、人数、车辆数、批次数等**不能停留在连续解**。取整后必须重新验证可行性；不可行时用明确的修复启发式，而不是直接四舍五入。

## 物理一致性
- 物理参数、单位和符号必须与建模报告一致。代码里不要出现与报告不一致的长度、质量、速度、半径、时间步长或坐标原点。

## 随机算法
- 随机算法要**固定随机种子**，并用多次运行（≥5 次）或稳定性指标说明结果可信度。报告结果均值和标准差，不要只报单次结果。

## 数据读取
- 数据读取后先检查编码、列名、形状、单位、缺失值和异常值。不要在数据错位或列名乱码时继续建模。

## 结果可追溯
- 所有论文会引用的数值必须能追溯到结果记录、JSON、表格或代码输出；不要在论文阶段重新估算或换一套四舍五入口径。
